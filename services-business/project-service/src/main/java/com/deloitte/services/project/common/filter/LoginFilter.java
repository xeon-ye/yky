package com.deloitte.services.project.common.filter;
...
import com.deloitte.services.project.common.util.ParameterRequestWrapper;
...
public class LoginFilter implements Filter {
    ...
    protected void toLogin(HttpServletResponse response) {
        ...
        try {
            responseJSONObject.put("code", "999999");
            responseJSONObject.put("url", "获取用户数据失败");
            out = response.getWriter();
            out.append(responseJSONObject.toString());

        } catch (IOException e) {
            log.error("用户读取数据失败！", e);
            throw new BaseException(PlatformErrorType.USER_EXCEPTION, e.getMessage(), e.getCause());
        } finally {
            if (out != null) {
                out.close();
            }
