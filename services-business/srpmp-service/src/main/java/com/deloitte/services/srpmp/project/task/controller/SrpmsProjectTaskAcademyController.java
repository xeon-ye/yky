 ...
 * @Modified :
 */
@Api(tags = "任务书-院基科费操作接口", value = "任务书-院基科费操作接口")
@Slf4j
@RestController
public class SrpmsProjectTaskAcademyController implements SrpmsProjectTaskAcademyClient {

    @Autowired
    public ISrpmsProjectTaskAcademyService srpmsProjectTaskService;
    ...
    public Result save(@RequestBody @ApiParam(name="SrpmsProjectTaskAcademyForm",value="新增SrpmsProjectTaskAcademy的form表单",required=true) SrpmsProjectTaskAcademyForm taskForm) {
    ...
    public Result submit(@Valid @RequestBody @ApiParam(name="SrpmsProjectTaskAcademyForm",value="新增SrpmsProjectTaskAcademy的form表单",required=true) SrpmsProjectTaskAcademyForm taskForm, UserVo userVo, DeptVo deptVo) {
    ...
    public Result export(@PathVariable(value = "id") Long id, HttpServletRequest request, HttpServletResponse response, UserVo userVo, DeptVo deptVo) {
        ...
        try {
            String downName = "医科院基科费项目任务书_" + DateFormatUtils.format(new Date(), "yyyyMMdd")+".docx";
            if (id != null && id.equals(0L)) {
                downName = "医科院基科费项目任务书模板.docx";
            }
    ...
    public Result exportPdf(@PathVariable(value = "id") Long id, HttpServletRequest request, HttpServletResponse response, UserVo userVo, DeptVo deptVo) {
        ...
        try {
            ...
            String downName = "医科院基科费项目任务书_" + DateFormatUtils.format(new Date(), "yyyyMMdd")+".pdf";
            if (id != null ) {
                downName = "医科院基科费项目任务书模板.pdf";
            }
            response.setHeader("Content-Disposition", "attachment;fileName=" + java.net.URLEncoder.encode(downName, "UTF-8"));
            is = new FileInputStream(fileUrl);
            bis = new BufferedInputStream(is);
    ...
    public Result<SrpmsProjectTaskAcademyVo> importWord(@Valid @RequestBody @ApiParam(name = "导入任务书", value = "WordImportReqVo", required = true) WordImportReqVo importReqVo) {
        SrpmsProjectTaskAcademyVo vo = srpmsProjectTaskService.importWord(importReqVo.getFileUrl());
        return new Result<SrpmsProjectTaskAcademyVo>().sucess(vo);
    }
}
